...
public class UploadConsultAction extends AbstractController {
...
//			super.setRedirect(false);
			...
			if(loginuser != null) {
				...
				// askcontent 는 밑에서 시큐어랑 같이 처리할 것
			...
			//	ConsultWriteVO_sm cWrite = new ConsultWriteVO_sm(fk_userid, category, name, email, mobile, asktitle, askcontent ); 
			//  밑에 것으로 바꾸기
				ConsultWriteVO_sm cWrite = new ConsultWriteVO_sm(fk_userid, category, name, email, mobile, asktitle, askcontent, ask_systemFileName, ask_orginFileName ); 
				
				
				
				// ###### 문의게시글 등록이 성공되면 문의게시글 리스트로 넘어감 ###### //
				try {
					
					InterConsultWriteDAO_sm cdao = new ConsultWriteDAO_sm();
					int n = cdao.uploadConsult(cWrite); // 메소드에 파일 관련 내용 추가
					
					if(n==1) {
						
						 String message = "문의글이 등록되었습니다! 게시판에서 확인해주세요 :-)";
