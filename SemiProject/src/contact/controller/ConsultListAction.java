...
public class ConsultListAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(loginuser != null ) {
			...
			List<ConsultWriteVO_sm> consultList = cdao.selectPagingConsult(paraMap);
			//  문의글이 여러개 나오므로 LIST 로 받기
			
			request.setAttribute("consultList", consultList);   // 메소드에서 받아온 값을 뷰단으로 보내주기 
			request.setAttribute("sizePerPage", sizePerPage);   // (선택한 페이지를 그대로 남겨주기 위한 처리)
			
			
			
			String pageBar = ""; // pagebar 는 consultList의 nav안의 li에 들어간다
			...
			int totalPage = cdao.getTotalPage(paraMap); // totalPage를 DB에서 가져올 것임
			...
			if( pageNo != 1 ) {
				// 현재 페이지가 1페이지만 아니라면, [맨처음]과 [이전] 버튼 사용가능
				 pageBar += "<li class='page-item'><a class='page-link' href='consultList.one?currentShowPageNo=1&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[맨처음]</a></li>"; 
				 pageBar += "<li class='page-item'><a class='page-link' href='consultList.one?currentShowPageNo="+(pageNo - 1)+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[이전]</a></li>"; 
			}
			...
			while( !(loop > blockSize || pageNo > totalPage) ) { // loop 가 blockSize 보다 커지면 반복문 멈춰(탈출)!  // totalPage 보다 pageNo가 커져도 반복문 그만
				...
				if( pageNo == Integer.parseInt(currentShowPageNo) ) {
					...
					pageBar += "<li class='page-item'><a class='page-link' href='consultList.one?currentShowPageNo="+pageNo+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>"+pageNo+"</a></li>"; 
			...
			if( pageNo <= totalPage ) {
				 pageBar += "<li class='page-item'><a class='page-link' href='consultList.one?currentShowPageNo="+pageNo+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[다음]</a></li>";
				 pageBar += "<li class='page-item'><a class='page-link' href='consultList.one?currentShowPageNo="+totalPage+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[마지막]</a></li>";
			}
			...
			// 로그인을 안한 경우 또는 일반사용자로 로그인한 경우
			 String message = "로그인한 유저만 접근이 가능합니다.";
